      ...
      do {
        ...
        switch (cpu.ir) {         // execute
		  ...
		  case PVM.lda_1: //new edit task 5 address encoded in opcode
			...
			if (inBounds(adr)) push (adr);
			break;
		  case PVM.lda_2: //new edit task 5 address encoded in opcode
			adr = cpu.fp -1 -2; //N is automatically 1
			if (inBounds(adr)) push (adr);
			break;
		  case PVM.lda_3: //new edit task 5 address encoded in opcode
			adr = cpu.fp -1 -3; //N is automatically 1
			if (inBounds(adr)) push (adr);
			break;			
		  case PVM.ldc_0: //new edit task 5 constant encoded in opcode
		    push(0); //N is automatically 0
            break;
		  case PVM.ldc_1://new edit task 5 constant encoded in opcode
		    push(1); //N is automatically 1
			break;
		  case PVM.ldc_2:
		    push(2);
			break;
		  case PVM.ldc_3:
		    push(3);
			break;
		  case PVM.ldl:           // new edit not working??
			adr = cpu.fp - 1 -next();
            if (inBounds(adr)) push(mem[adr]);
            break;
		  case PVM.ldl_0: //new edit task 5 load and dereference variable 0
		    adr = cpu.fp-1;
			if (inBounds(adr)) push (mem[adr]);
			break;
		  case PVM.ldl_1://new edit task 5 load and dereference variable 1
		    adr = cpu.fp-1-1; 
			if (inBounds(adr)) push (mem[adr]);
			break;
		  case PVM.ldl_2://new edit task 5 load and dereference variable 2
		    adr = cpu.fp-1-2; 
			if (inBounds(adr)) push (mem[adr]);
			break;	
		  case PVM.ldl_3://new edit task 5 load and dereference variable 3
		    adr = cpu.fp-1-3; 
			if (inBounds(adr)) push (mem[adr]);
			break;			
		  case PVM.stl:	//new edit task 5 load variable N and store tos in mem[N]
		    adr = cpu.fp - 1 -next();
			if (inBounds(adr)) mem[adr] = pop();
			break;
		  case PVM.stlc: // new edit task 5 store character in variable N -- NEEDS TESTING
			adr = cpu.fp - 1 -next();
			tos = pop(); 
            if (inBounds(adr)){
              if (tos >= 0 && tos <= maxChar) mem[adr] = tos;
            ...
            break;
		  case PVM.stl_0:	//new edit task 5 store tos in variable 0
		    adr = cpu.fp -1 -0;
			if (inBounds(adr)) mem[adr] = pop();
			break;
		  case PVM.stl_1: //new edit task 5 store tos in variable 1
		    adr = cpu.fp-1-1;
			if (inBounds(adr)) mem[adr] = pop();
			break;
		  case PVM.stl_2: //new edit task 5 store tos in variable 1
		    adr = cpu.fp-1-2;
			if (inBounds(adr)) mem[adr] = pop();
			break;		  
		  case PVM.stl_3: //new edit task 5 store tos in variable 1
		    adr = cpu.fp-1-3;
			if (inBounds(adr)) mem[adr] = pop();
			break;		  
